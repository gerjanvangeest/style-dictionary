...
/**
 * Overrides filePath with local context and adds 'appliedFrom'
 *
 * @context
 * // source
 * {
 ...
 * }
 *
 * @param {object} resolvedObj the result of refObj: "external.ref"
 * @param {object} refObj the object that '@apply' points to: `{ "value" : "{external.ref}" }`
 * @param {string} unresolvedValue
 * @param {object|null} obj the object that wraps '@apply': "myComponent"
 */
function adoptLocalContextResolvedObject(
  resolvedObj,
  refObj,
  unresolvedValue,
  obj
  ...
  const keysOriginalObj = obj ? Object.keys(obj) : null;
  for (const key in resolvedObj) {
    if (key.startsWith("@")) {
      continue; // skip decorators
    }
    const shouldOverride = keysOriginalObj
      ? keysOriginalObj.indexOf("@apply") > keysOriginalObj.indexOf(key)
      : true;
    if (shouldOverride) {
      const appliedFrom = `${unresolvedValue}.${key}`;
      let toBeMerged = resolvedObj[key];
      if (_.isPlainObject(resolvedObj[key]) && !("value" in resolvedObj[key])) {
        toBeMerged = adoptLocalContextResolvedObject(
          resolvedObj[key],
          refObj,
          appliedFrom,
          null
      ...
      result[key] = Object.assign({}, toBeMerged, {
        filePath: refObj.filePath,
        appliedFrom,
      });
    }
  }
  ...
  if ("@apply" in obj) {
    ...
    if (_.isPlainObject(refObj) && "value" in refObj) {
      ...
      if (_.isPlainObject(resolvedObj) && _.isPlainObject(resolvedObj.value)) {
        ...
        // Merge toBeMerged with obj
        for (const key in toBeMerged) {
          obj[key] = toBeMerged[key];
        }

        // Now delete the original "@apply", because we don't want it to resolve in next iterations.
        // Instead, add back some meta info for documentation, so that we can derive what tokens
        // are built from. (For instance, show a table that component token 'SwitchThumb' consumes
        // the more generic token 'InputThumb', which in turn consists of mutliple tokens)
        obj._applied = {
          // The value {x.y.z}. Adding this prop allows to
          // iterate over prop in our custom formatters/transforms/filters
          value: unresolvedValue,
          // referenceObject: resolvedObj.value, // the resolved object
        };
      ...
      console.warn(
        `@apply: should contain { value:string }, but it contained: ${JSON.stringify(
          refObj
        )}`
      );
    }
...
function exportPlatform(platform) {
  ...
  // Merge @apply references into exportableResult. Be sure to run before enhanceWithGroupMeta, so group enhancements
