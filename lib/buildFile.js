 ...
 * @returns {null}
 */
function buildFile(destination, format, platform, dictionary, filters) {
  ...
  var filteredProperties = filterProperties(dictionary, filters);
  ...
  Object.keys(nameCollisionObj).forEach((propertyName) => {
    if (nameCollisionObj[propertyName].length > 1) {
      let collisions = nameCollisionObj[propertyName]
        .map((properties) => {
          let propertyPathText = chalk.keyword("orangered")(
  ...
  if (propertyNamesCollisionCount > 0) {
    ...
    let title = `While building ${chalk
      ...
      .bold(
        destination
      )}, token collisions were found; output may be unexpected.`;
